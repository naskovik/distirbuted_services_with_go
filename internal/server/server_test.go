...
import (
	...
	"github.com/naskovik/proglog/internal/auth"
	"github.com/naskovik/proglog/internal/config"
	"github.com/naskovik/proglog/internal/log"
...
func testProduceConsumeStream(t *testing.T, client, _ api.LogClient, config *Config) {
	ctx := context.Background()
	records := []*api.Record{{
		Value:  []byte("first_message"),
		Offset: 0,
	}, {
		Value:  []byte("second_message"),
		Offset: 1,
	}}
	{
